...
public interface MutualFundTransactionRepository extends JpaRepository<MutualFundTransactionEntity, Long> {
	...
	void addMutualFundTransaction(Long userId, Long mutualFundsId, Double price, Double unit);

	@Modifying
	@Query(value = "UPDATE mutual_fund_transaction SET unit = unit + ?1 WHERE user_id = ?2 AND mutual_funds_id = ?3", nativeQuery = true)
	void updateMutualFundTransaction(Double unit, Long userId, Long mutualFundId);

	@Query(value = "SELECT id FROM user WHERE username = ?", nativeQuery = true)
	Long getById(String username);

	@Query(value = "SELECT unit FROM mutual_fund_transaction WHERE user_id = ?2 AND mutual_funds_id = ?1", nativeQuery = true)
	double check(Long mutualFundsId, Long userId);
