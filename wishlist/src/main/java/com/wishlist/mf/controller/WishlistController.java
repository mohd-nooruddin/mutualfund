...
public class WishlistController {
    ...
    public String addWishlistItem(@RequestParam("customerId") Long customerId, @RequestParam("mutualfundId") Long mutualfundId) {
        if (wishlistService.check(customerId, mutualfundId) == 0) {
        	MutualFunds funds = wishlistService.findId(mutualfundId);
        	wishlistService.insertWishlistItem(customerId, mutualfundId, funds.getFundHouse(), funds.getSchemaName(), funds.getCurrentPrice(), funds.getDelta());
